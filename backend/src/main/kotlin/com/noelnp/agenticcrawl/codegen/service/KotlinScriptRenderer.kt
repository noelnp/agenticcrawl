...
class KotlinScriptRenderer {
    ...
    private fun renderTopLevelStep(sb: StringBuilder, step: ExtractionStep) {
        ...
        when (step) {
            ...
            is ExtractionStep.WaitForSelector -> {
                ...
                sb.append("        if (ensureLocated(page, \"${esc(step.selector)}\") == 0) throw e\n")
                sb.append("        log(\"waitForSelector('${esc(step.selector)}') recovered via scroll fallback\")\n")
                sb.append("    }\n")
            }
    ...
    private fun renderTopLevelExtractRows(sb: StringBuilder, step: ExtractionStep.ExtractRows) {
        ...
        sb.append("    ensureLocated(page, \"${esc(step.rowSelector)}\")\n")
        ...
        for (step in steps) {
            ...
            when (step) {
                ...
                is ExtractionStep.WaitForSelector -> {
                    ...
                    sb.append("$indent    if (ensureLocated(detailPage, \"${esc(step.selector)}\") == 0) throw e\n")
                    sb.append("$indent    log(\"waitForSelector('${esc(step.selector)}') recovered via scroll fallback\")\n")
                    sb.append("${indent}}\n")
                }
    ...
    private fun renderInnerExtractRows(
        ...
        if (scope == "detailPage") {
            sb.append("${indent}ensureLocated($scope, \"${esc(step.rowSelector)}\")\n")
        }
    ...
    private companion object {
        ...
        const val POST_NAV_SETTLE_MS = 3_000
...
// Some content sits just below the fold or lazy-loads on scroll. Scroll a
// little and re-check, up to a few attempts, before giving up.
fun ensureLocated(page: Page, selector: String, maxScrolls: Int = 3, scrollPx: Int = 600): Int {
    var count = runCatching { page.locator(selector).count() }.getOrDefault(0)
    if (count > 0) return count
    for (i in 0 until maxScrolls) {
        runCatching {
            page.evaluate("window.scrollBy(0, ${'$'}scrollPx)")
            page.waitForTimeout(500.0)
        }
        count = runCatching { page.locator(selector).count() }.getOrDefault(0)
        if (count > 0) {
            if (debug) log("ensureLocated('" + selector + "') found after " + (i + 1) + " scroll(s)")
            return count
        }
    }
    return 0
...
fun extractField(scope: Locator, selector: String, nth: Int?, source: Source): String? {
    val page = runCatching { scope.page() }.getOrNull()
    for (attempt in 0..2) {
        try {
            val base = scope.locator(selector)
            val matches = base.count()
            val sufficient = matches > 0 && (nth == null || nth < matches)
            if (sufficient) {
            ...
            if (debug) log("extractField('" + selector + "') attempt " + attempt + " failed: " + e.message)
        }
        if (attempt < 2 && page != null) {
            runCatching {
...
fun dismissConsent(page: Page) {
    ...
    val patterns = listOf(
        ...
        "Accept all", "Accept", "I agree", "Got it", "OK", "Tillåt alla"
    )
